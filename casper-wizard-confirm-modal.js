...
class ConfirmWizardConfirmModal extends Casper.I18n(CasperWizard) {
  ...
  ready () {
    ...
    this.addEventListener('accept-modal', () => this._resolve && this._resolve({ ok: true, reason: this.reason }) );
  ...
  setOptions (options) {
    ...
    if (options.needs_reason) {
      this.defaultOverrideWizardDimensions = { width: '450px', height: '230px' };
    } else {
      this.defaultOverrideWizardDimensions = { width: '350px', height: '200px' };
    }
  ...
  connectedCallback () {
    ...
    afterNextRender(this, () => {
      if (this.shadowRoot.querySelector('paper-input')){
        ...
        this.shadowRoot.querySelector('paper-input').addEventListener('keydown', e => this._onKeyDown(e))
      }
    });
  }
